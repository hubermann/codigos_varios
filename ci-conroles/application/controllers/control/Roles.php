...
public function index(){
 # $this->permiso->verify_access( 'roles', 'view');
  ...
  if(!$page){ $start =0; $page =1; }else{ $start = ($page -1 ) * $per_page; }
    ...
    $total_pages = ceil($this->role->count_rows() / $per_page);
  ...
  $data['query'] = $this->role->get_records($per_page,$start);
...
public function detail(){
  ...
  $data['menu'] = 'control/roles/menu_rol';
  $data['query'] = $this->role->get_record($this->uri->segment(4));
  $this->load->view('control/pixel-admin/control_layout', $data);
}
...
public function form_new(){
  ...
  #echo $this->permiso->verify_access( 'roles', 'create');
  $this->load->helper('form');
  $data['title'] = 'Nuevo rol';
  $data['content'] = 'control/roles/new_role';
  $data['menu'] = 'control/roles/menu_rol';
  $this->load->view('control/pixel-admin/control_layout', $data);
}
...
public function editar(){
  ...
  $data['content'] = 'control/roles/edit_role';
  $data['menu'] = 'control/roles/menu_rol';
  $data['query'] = $this->role->get_record($this->uri->segment(4));
  $this->load->view('control/pixel-admin/control_layout', $data);
}
...
$this->form_validation->set_rules('nombre', 'Nombre', 'required');
  ...
  if ($this->form_validation->run() === FALSE){
    ...
    $this->role->update_record($id, $editedrol);
...
public function soft_delete(){
  ...
  if($id_rol > 0 && $id_rol != ""){
    ...
    $this->role->update_record($id_rol, $editedrol);
